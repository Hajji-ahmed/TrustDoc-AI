...
export function DocumentPreview({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-lg border border-[var(--color-line)] bg-[var(--color-canvas)] p-2">
      ...
      <div className="mt-2 flex shrink-0 items-center justify-between gap-3 text-xs text-[var(--color-ink-muted)]">
        <span className="truncate">{document.fileName}</span>
        <span className="shrink-0">
          {document.width} × {document.height} px
          {document.pageCount > 1 ? ` — page 1/${document.pageCount}` : ''}
        </span>
      </div>

      {drawable.length > 0 ? (
        <p className="mt-1 shrink-0 text-xs text-[var(--color-ink-muted)]">
